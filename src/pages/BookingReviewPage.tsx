...
function BookingReviewPage() {
  ...
  return <main className="page-shell review-page"><Link to={`/book/${draft.gameType}`} className="back-link">← Change</Link><header className="compact-heading"><h1>Your booking</h1></header><div className="review-layout">
    <section className="review-card">{reservations.map((reservation, index) => <div className="review-session" key={reservation.time}><span className="review-session__number">{index + 1}</span><div><strong>{game.name}</strong><span>{formatDateLabel(reservation.date)} · {reservation.time}–{formatEndTime(reservation.time)}</span></div><b>{formatMoney(game.price)}</b></div>)}<Link to="/" className="add-link">+ Add another game</Link></section>
    <section className="review-card review-form"><label className="field"><span className="field__label">Your name</span><input autoComplete="name" autoFocus className="field__input" value={customerName} onChange={(event) => { setCustomerName(event.target.value); setError('') }} placeholder="Name" /></label><div className="review-total"><span>Total</span><strong>{formatMoney(total)}</strong></div>{error && <p className="form-error" role="alert">{error}</p>}<button className="button button--primary button--wide" type="button" onClick={submit}>Confirm booking</button><small className="pay-note">Pay at reception.</small></section>
  </div></main>
}
