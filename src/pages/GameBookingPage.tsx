...
function GameBookingPage() {
  ...
  return <main className="page-shell booking-page">
    ...
    <section className="booking-section"><div className="section-title-row"><h2>Time</h2><span>{selectedTimes.length}/2 selected</span></div><p className="pay-note">1-hour sessions, starting every 30 minutes. Times after midnight are on the following day.</p>{(loading || data.date !== selectedDate) && !error && <p role="status">Loading availability...</p>}{error && <p className="form-error" role="alert">{error} <button type="button" onClick={refresh}>Retry</button></p>}{!loading && !error && data.date === selectedDate && !inventory && <p role="status">This game is currently unavailable.</p>}<div className="time-list">
