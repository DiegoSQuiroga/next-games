...
import { useBookingGroup } from '../hooks/useBookingData'
...
function BookingStatusPage() {
  ...
  const { data: group, loading, error, refresh } = useBookingGroup(reference ?? '')
  if (loading) return <main className="page-shell"><p role="status">Loading booking...</p></main>
  if (error) return <main className="page-shell"><p className="form-error" role="alert">{error}</p><button className="button button--secondary" onClick={refresh}>Retry</button></main>
